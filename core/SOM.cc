...
{
  //cout << "SelfOrgFeatMap::Train" << endl;
...
int SelfOrgFeatMap::BestMatch(svec<double> & feat, svec<int> & valid)
{
  ...
  //cout << "SelfOrgFeatMap::BestMatch" << endl;
...
bool SelfOrgFeatMap::Serialize(svec<double> & out) const
{
  int i, j;
  int n = m_nodes.isize()*m_nodes[0].GetCoords().isize();
  out.resize(n, 0.);
  int k = 0;
  for (i=0; i<m_nodes.isize(); i++) {
    const svec<double> & d = m_nodes[i].GetCoords();
    for (j=0; j<d.isize(); j++) {      
      out[k] = d[j];
      k++;
    }
