...
class SOMNode
{
 public:
  ...
  void Update(svec<double> & feat, svec<int> & valid, double distance, double weight = 1.) {
    ...
    //cout << "Update, m_extCoords=" << m_extCoords.isize() << endl;

    for (int i=0; i<m_extCoords.isize(); i++) {
      if (valid[i] > 0) {
	//cout << i << " " << distance << " " << m_extCoords[i] << " -> ";
  ...
  void UpdateDepend(svec<double> & feat, svec<int> & valid, double weight) {
    for (int i=0; i<m_connect.GetNodeCount(); i++) {
      ...
      //cout << "UpdateDepend: " << i << " count " << m_connect.GetNodeCount() << endl;
      SOMNode * p = m_connect.GetNode(i);
      //cout << "Call update, feat=" << feat.isize() << endl;
  ...
  const svec<double> & GetCoords() const {return m_extCoords;}
...
class SelfOrgFeatMap
{
 public:
  ...
  bool Serialize(svec<double> & d) const;
